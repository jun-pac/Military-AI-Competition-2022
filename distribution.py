...
tot_cnt2 = 0
tot_cnt3 = 0
for c0, c1, c2, c3 in result:
    tot_cnt0 += c0
    tot_cnt1 += c1
    tot_cnt2 += c2
    tot_cnt3 += c3
tot_sum = tot_cnt0 + tot_cnt1 + tot_cnt2 + tot_cnt3
f_log = open("/home/oiocha/maicon/sejun/distibution.txt", "w+")
print(
    f"First {N} datas | tot_cnt0 : {tot_cnt1}({tot_cnt0 / tot_sum * 100:.2f}%), tot_cnt1 : {tot_cnt1}({tot_cnt1 / tot_sum * 100:.2f}%), tot_cnt2 : {tot_cnt2}({tot_cnt2 / tot_sum * 100:.2f}%), tot_cnt3 : {tot_cnt3}({tot_cnt3 / tot_sum * 100:.2f}%)")
f_log.write(
    f"First {N} datas | tot_cnt0 : {tot_cnt1}({tot_cnt0 / tot_sum * 100:.2f}%), tot_cnt1 : {tot_cnt1}({tot_cnt1 / tot_sum * 100:.2f}%), tot_cnt2 : {tot_cnt2}({tot_cnt2 / tot_sum * 100:.2f}%), tot_cnt3 : {tot_cnt3}({tot_cnt3 / tot_sum * 100:.2f}%)\n")
f_log.flush()